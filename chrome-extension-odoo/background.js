/* WFM Odoo Bridge — service worker.
 * Receives captured Odoo records from content.js and POSTs them to the WFM backend
 * (/api/v1/integrations/odoo/push). Auth model mirrors the Sprinklr/Ameyo bridges:
...
async function pullFromTabs() {
  try {
    const tabs = await chrome.tabs.query({});              // Odoo domain is unknown → try any tab; content replies only if it's Odoo
    for (const tab of tabs) {
      if (!tab.id || !/^https?:/.test(tab.url || '')) continue;
      try {
        const res = await chrome.tabs.sendMessage(tab.id, { type: 'PULL_ODOO' });
        if (res?.ok && res.snapshot) { await chrome.storage.local.set({ lastHeartbeat: Date.now(), odooUrl: tab.url }); await handleSnapshot(res.snapshot); return; }
      } catch { /* not an Odoo tab / no content script */ }
    }
  } catch { /* ignore */ }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'WFM_ODOO_SNAPSHOT') { handleSnapshot(msg.snapshot).then((r) => sendResponse(r)); return true; }
...
async function pushToWfm(snapshot, isRetry = false) {
  ...
  try {
    const res = await fetch(`${config.wfmApiUrl}/integrations/odoo/push`, {
      method: 'POST',
    ...
    if (res.ok) {
      ...
      await chrome.storage.local.set({ lastPushAt: Date.now(), lastPushStatus: 'ok', lastPushCount: snapshot.count ?? 0 });
      setBadge(`${snapshot.count ?? 0}`, '#22c55e');
      return { ok: true, ...body };
    }
...
async function getStatus() {
  const d = await chrome.storage.local.get(['config', 'lastSnapshot', 'lastSnapshotAt', 'lastPushAt', 'lastPushStatus', 'lastHeartbeat', 'lastPushCount', 'odooUrl', 'wfmRefreshToken', 'lastLoginAt']);
  ...
  return {
    ...
    recordCount: d.lastPushCount ?? 0, odooUrl: d.odooUrl ?? '',
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  setBadge('OFF', '#64748b');
  console.log('[WFM Odoo Bridge] Installed.');
});
