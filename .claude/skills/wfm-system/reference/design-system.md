...
Reusable, **theme-aware** (reads `var(--surface/-2)`, `var(--border)`, `var(--text-1/2/3)` → correct in
Dark/Light/Glass automatically) and **reduced-motion-respecting** primitives — the canonical way to add
premium motion. PREFER over hand-rolled tiles/bars for new KPI/visual work.
- `useReducedMotion()`, `useCountUp(target, ms?, run?)` (ease-out, snaps if reduced, keeps 1 decimal).
- `<StatTile icon label num|value suffix? sub? color delay? onClick?/>` — count-up KPI tile with accent
  top-edge, corner glow, hover-lift, staggered entrance.
- `<Donut segments[{label,value,color}] centerNum centerSuffix? centerLabel/>` — animated SVG ring +
  count-up center + % legend (e.g. the disjoint regular/off-day/holiday OT split).
- `<BarRow label value max color suffix? delay?/>` — growing horizontal bar.
Applied to OT-Exceptions, RosterDashboard, Agent360; verified in all three themes.

## Light-mode net — the inline-dark-HEX gap (closed 2026-06-23)

The `.theme-light` net (in `index.css`) matches the **serialized** form of inline styles — React turns
`style={{color:'#f1f5f9'}}` into `style="color: rgb(241, 245, 249)"`, and the net targets that `rgb()`
substring. It covered `text-white`/`text-slate-*`/`bg-white/`/`border-white/` + translucent
`rgba(255,255,255,0.0x)` — but NOT inline solid near-black HEX backgrounds (`#11162a`, `#0f1527`,
`#0f172a`, `#0b0f1c`, `#0b1120`, `#080f1a`, `rgba(7,9,15,…)`) used by ~12 report pages for table heads →
stayed black in light. Fixed with `.theme-light [style*="background: rgb(17, 22, 42)"]…{ background:#e9edf4
!important }` per near-black rgb form (scoped to `.theme-light`; Glass stays dark). **New pages: use CSS
vars / the dazzle kit; don't hardcode a dark hex — if you must, add its serialized `rgb()` to the net.**

## Schedule grid — reconciliation visual language (2026-06-30)
The Schedule grid cell (`Schedule.tsx` ShiftCell) reads the corrected `roster_days` overlay and encodes status:
- **Shift code DIRECT** (`E/M/B/C/MD/C7…`) — **no `-WFH` suffix** (user pref). WFH = 🏠 icon + a **dotted texture**
  (`backgroundImage: radial-gradient(circle at 2px 2px,<text>33 1px,transparent 1.6px); backgroundSize:7px 7px`).
- **Reconciled vs Planned** — corrected (`source:'roster'`) = solid; planned/future (`source:'schedule'`) = dashed +
  opacity 0.8.
- **Holiday** — gold top ribbon (`boxShadow: inset 0 2.5px 0 0 #fbbf24, 0 0 10px rgba(251,191,36,0.22)`).
- **Time must come from the same source as the code** — from `roster_days.shift_start_min/shift_end_min` (canonical),
  not stale `attendance_records.scheduled_start`; mixing sources is a bug (once showed MD as "7am–4pm").
