...
## 21. Smart Break Management (BR-BRK)

> Source for the whole section: **Director spec 2026-07-10 (`SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md`, repo root, commit 3d1d82e)**, executed B1–B5 (commits 1801344 · 9cf3c0e · 033f373 · 0f5fa44). Canonical rule text: RULES §23. Module: `backend/src/modules/breaks/`; data model: migration `database/migrations/079_break_policies_v2.sql` (m079).

| ID | Rule | Status | Source | Enforced in |
|---|---|---|---|---|
| BR-BRK-001 | **Daily break entitlement = max 4 sessions / 60 total minutes**, enforced in BOTH request paths (breaks module + requests-module bridge) with bilingual rejects. Caps + distribution pattern ([15,15,15,15] default) are configurable per policy row — never hardcoded. `sessionPattern` trims/cuts trailing sessions so neither cap can be exceeded. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §1 | `computeEntitlement` (break-policy.logic.ts:90-114); `enforceEntitlement` (break-policy.service.ts:92, called from breaks.service.ts:293 + requests.service.ts:1371); `sessionPattern` (break-policy.logic.ts:60-76); m079 CHECK constraints |
| BR-BRK-002 | **Policy resolution = most-specific-wins** over the `break_policies_v2` matrix: NULL selector = wildcard; specificity score function=4 + shift_type=2 + employment_type=1; only active rows with every non-NULL selector matching are candidates. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §1/§21 | `pickMostSpecificPolicy` (break-policy.logic.ts:35-56); m079 `break_policies_v2` |
| BR-BRK-003 | **Protected first + last shift hour:** no AUTO break may start within `protected_first_min` (default 60) or end within `protected_last_min` (default 60) of the shift. Protected-window breaks require a MANUAL exception (reason + coverage impact + authorized approval); entitlement is never silently exceeded — an override is a separate audited action (`breaks.entitlement.override`). | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §2 | `generationWindow` (break-policy.logic.ts:122-131); scheduler protected-window warnings (break-scheduler.service.ts:387,407); override audit (breaks.service.ts:298, requests.service.ts:1376) |
| BR-BRK-004 | **Function-level calculation + coverage floor source:** all capacity/risk math is PER FUNCTION (canon_fn). Coverage floor = `required_hc` from `headcount_intervals`, auto-rebuilt in-process when empty; if still empty the engine falls back to required=scheduled and DISCLOSES `coverageSource:'fallback'` — never a silent assumption. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §4/§6 | break-scheduler.service.ts:233-270 (`CoverageRebuildService` delegate + disclosed fallback) |
| BR-BRK-005 | **Anti-clustering caps:** per-function per-15-min simultaneous-break cap (explicit `thresholds.max_simultaneous` or computed scheduled−required−buffer) AND per-`team_manager` cap (default 1), enforced at generation AND at live release. AS-BUILT scope: function + team caps only (skill/language/seniority clusters deferred). | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §9 | `functionSimultaneousCap` (break-policy.logic.ts:164-179); `antiClusterOk` (break-release.logic.ts:368-376); `maxSimultaneousBreaks` (break-release.logic.ts:143-158) |
| BR-BRK-006 | **Release modes = auto / supervisor / hybrid / freeze** (per policy row, switchable live). Red/critical risk = HOLD in every non-frozen mode; supervisor mode never auto-releases; hybrid routes protected-window / entitlement-override / orange-risk cases to supervisor-recommend; freeze releases nothing. **Every mode change is audited.** | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §13 | `decideRelease` (break-release.logic.ts:347-355); `setMode` + audit `breaks.engine.mode` (break-release.service.ts:771-778); `POST /breaks/engine/mode` (breaks.controller.ts:131-140) |
| BR-BRK-007 | **Risk ladder green→yellow→orange→red→critical with shown reasons** (simulates releasing ONE more employee: coverage after release, queue waiting, SLA-risk queues, live floor, next-30-min forecast). **Stale fail-safe:** snapshot >5 min degrades one level (worse only, never better); >15 min or NO snapshot = orange floor (supervisor-confirmation). Never assume stale data is safe. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §5/§23/§30 | `riskAssess` (break-release.logic.ts:73-136) |
| BR-BRK-008 | **Priority is explainable, never a black box:** per-factor AR/EN breakdown `{points, reason, reasonAr}` shown to supervisor and agent; weights configurable per policy (`thresholds.priority_weights`). **Penalties are TODAY-only** (recently-returned, above-median-sessions) — no permanent punishment. Ordering deterministic: score desc → earlier planned start → slotId. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §7/§22 | `priorityScore` + `orderCandidates` (break-release.logic.ts:232-326); breakdown rendered in CommandCenter waiting line |
| BR-BRK-009 | **Delay transparency + escalation ladder:** a delayed break is never silently pending — status, delay reason and updated ETA shown to the employee; configurable ladder (default 10/20/30/max-delay min) notifies → raises priority (+1/min waited, capped) → alerts supervisor; restart-safe stage dedup; audited (`breaks.delay.escalated`). | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §11 | `delayLadder`/`delayStage` (break-release.logic.ts:381-395); escalation audit (break-release.service.ts:837); wait bonus in `priorityScore` |
| BR-BRK-010 | **An agent cannot start an automatic break before the system releases it** — `POST /breaks/slots/:id/start` returns 400 until status=released. Manual supervisor release requires a REASON at risk ≥ orange and is audited (`breaks.release.manual`); auto releases audited too (`breaks.release.auto`). | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §14/§16 | `startBreak` (breaks.controller.ts:113-119 → break-release.service.ts); `manualRelease` (break-release.service.ts:659-698); auto-release audit (break-release.service.ts:271) |
| BR-BRK-011 | **Return monitoring:** reminder at break-end−5 min; `overdue` after end+5 min grace; actual consumed minutes posted to `break_daily_balance` on return (balance = MAX(ledger, live slots) so neither a stale ledger nor an unposted completion overdraws). Late returns reportable; **future breaks NEVER auto-extended to compensate**. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §19 | Return-monitoring phase (break-release.service.ts:290-314, `RETURN_REMINDER_MIN=5`); `returnFromBreak` (break-release.service.ts:729-760); m079 `break_daily_balance` |
| BR-BRK-012 | **Cross-midnight slot dating:** a slot whose wall-clock start crosses midnight belongs to the NEXT calendar day via `planned_date` (fixed the date-loss bug; 4,985 slots backfilled in m079). `break_daily_balance` uses a full TIMESTAMPTZ shift window → cross-midnight safe. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §21; RULES §19 (start-day ownership) | `plannedDateFor` (break-policy.logic.ts:140-147); m079 `break_slots.planned_date` backfill |
| BR-BRK-013 | **Simulation never writes:** `POST /breaks/simulate` dry-runs the SAME optimizer (`generateForDate`) and the SAME `riskAssess` with scenario overlays (deterministic FNV-1a absence, queue spike, extra staff); zero persistence proven (identical slot counts on double-run); UI badges "SIMULATION — not applied". | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §28 | break-simulation.service.ts:60 (dry-run call) + kept-slots overlay (:149-157); breaks.controller.ts:66-73 |
| BR-BRK-014 | **Full audit coverage:** `breaks.release.auto` / `breaks.release.manual` / `breaks.start` / `breaks.return` / `breaks.overdue` / `breaks.delay.escalated` / `breaks.engine.mode` / `breaks.policy.updated` / `breaks.entitlement.override` / `breaks.actual.recorded` — no silent manual reordering; manual actions carry a reason. | Confirmed | Director spec 2026-07-10 (SMART_DYNAMIC_BREAK_MANAGEMENT_PROMPT.md) §16/§26 | break-release.service.ts:271,314,698,723,760,778,837; breaks.controller.ts:187-191; breaks.service.ts:298 |

---

## 22. Decisions Register (selected, dated)
...
| D-078 | Smart Break engine (Director's 32-section spec) EXECUTED B0–B5; accounts provisioning deliberately deferred to LAST (pre-rollout step); recommended rollout = hybrid-mode pilot on one function. | 2026-07-11 | RULES §23; DECISIONS_AND_AGREEMENTS_LOG D-078 |

## 23. Known Drifts & Risks (open — do not treat as rules)
