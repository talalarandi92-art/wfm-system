...
### D-077 — Generators behavioral merge EXECUTED (demand-driven engine = THE generator)
- **Status:** Confirmed (approved by the Director 2026-07-08 — "موافق كمل"; executed same date)
- **Decision:** `generateDemandDriven` (demand.engine.ts) is THE generator. The classic engine's two Director-tested structures are grafted in as OPTIONS: `offStrategy:'weekend-fair'` (one weekend Thu/Fri OFF + one mid-week OFF, weekend slot rationed by YTD weekend-OFF fairness, reusing the classic `assignOffDays`; surplus people WORK as labeled overstaffing so the structure never erodes) and `rotationFairness:true` (weekly band target night→afternoon→morning→midnight, females morning↔afternoon only, as the leading fairness tiebreak). Defaults unchanged (`lowest-demand`, rotation off) — the Director-tested demand behavior stays the default. Classic `/generate` remains for comparison; retirement after UI repoint (P1 tail).
- **Edge-hour defect fixed first (pre-merge investigation):** `computeShiftMix`'s greedy favored mid-day shifts (B/C) and stranded window edges — 08:00 coverable only by M, 18–20 only by C/N/EE — and could place redundant bodies. Added a deterministic edge-patch pass (drop fully-redundant shifts → 1-for-1 swaps that strictly reduce total then worst deficit → greedy refill). Verified on week 2026-07-11: residual gap intervals 358→324, total deficit person-slots 1062→974, Refund/SM&E/Offline 08:00 edge gaps eliminated; remaining Support/CC edges are true pool infeasibilities.
- **hiring-now consistency (the "Refund −2@08:00 while peak said 2" confusion):** additive transparency fields `scheduleBodiesWorstDay` / `fieldablePerDay` / `coverageGapBodies` (uncapped shift-mix over the hourly curve vs pool−2-OFF/wk) — the schedulable-team view that predicts generator residuals. Headline `internsToHire` stays keyed to requiredPeak (changing it needs the Director's go).
- **Verified:** `npm run verify` PASS; 36/36 engine specs (7 new); live before/after week 2026-07-11 — female MD/MN/E/EE = 0 in all runs; 104/104 scheduled employees exactly 2 OFF; weekend-fair run: 92 with the exact 1-weekend+1-midweek shape, 12 exceptions all legitimate MAX_CONSEC forced-rest relocations (classic parity), 0 both-weekend.
- **Source:** session 2026-07-08 (commit in repo); memory `staffing_requirement_engine`; EXECUTION_BRIEF Phase-1 consolidate note.
