...
| L-012 | **OT credited the raw system span, not the scheduled net (2026-07-05 8-lens roster_days audit, 34 agents)** — the roster is trustworthy for who/when/where (identity clean, WFH 100% signal-backed), but OT/tardiness had a soft edge: holiday OT over the net cap, punch-derived OT above the 5h ceiling, absence-suffixed codes earning OT, OFF-day system spans wrapping to 12–31h, cross-midnight late/early >240m (logout bleed) driving conformance. **Key realisation: most defects were STALE rows from OLDER builds** (Jan–May = `import-roster-master.js`; older June recon), not the current engine — a fresh rebuild clears them. | **Engine hardening in `recon-build.js` (commit a33fb22, code-only):** `otNetCap` caps holiday/off OT at scheduled net (fallback 480/360) on every path; `OT_CEIL=300` caps `ot_min` on every basis incl. punch; `otEligible` blocks OT on absence/sick/sep/unmapped; cross-midnight `sys_late/early` capped at 240 + bleed flag; login-only OT gets a soft `data_quality` flag; `recon-ingest.js` now maps `permission_status`. Smart principle (from L-shift-inference training): **schedule=truth, system=deviation, logout bleeds → OT = scheduled net worked.** All enforce EXISTING agreed rules (BR-OT-003/004/001, BR-TRD) — no new rule. Dry-run PASS, adversarial review SAFE, 93/96/95% calibration unaffected. ⏳ Full apply awaits fresh June sources → dry-run diff + rebuild; route all months through recon. | WFM_RULES_AND_DECISIONS.md §12/§19; memory `audit_2026_07_05_roster_accuracy_engine_hardening` |
...
| L-013 | **Greedy shift-mix strands operating-window edges (2026-07-08, pre-merge investigation)** — `computeShiftMix` picked mid-day-heavy shifts (B/C, 18 in-window slots each) before edge shifts, but 08:00 is coverable ONLY by M and 18–20 only by C/N/EE; with small pool ceilings this produced −2@08:00 while `hiring-now` said requiredPeak covered (the Refund confusion). Greedy also placed redundant bodies (5 where 4 cover 2-flat over 08–20: 2×M+2×C). Peak-vs-team is NOT a schedulability check: a 12h window at requirement 2 needs 4 scheduled bodies/day. | Deterministic **edge-patch pass** after the greedy (drop fully-redundant shifts → 1-for-1 swaps that strictly reduce total then worst deficit → greedy refill; terminates by monotone improvement). Week 2026-07-11: residuals 358→324 intervals / 1062→974 person-slots; Refund/SM&E/Offline 08:00 gaps eliminated. `hiring-now` gained the schedulable view (`scheduleBodiesWorstDay`/`fieldablePerDay`/`coverageGapBodies` — uncapped mix vs pool−2-OFF/wk); headline unchanged pending the Director. | D-077; `demand.engine.ts` computeShiftMix; `staffing.service.ts` hiringNow; memory `staffing_requirement_engine` |
