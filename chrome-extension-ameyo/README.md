# WFM Bridge — Ameyo Connector

Reads the Ameyo **Live Monitoring** screen (agents list + KPI cards) and syncs it
...
1. **Network interceptor** (`injected.js`) wraps `fetch`/XHR/WebSocket in the page
   world → sees how Ameyo loads its live data (it refreshes ~every 10s).
...
3. Open the **Ameyo Live Monitoring** page. The badge turns green with the agent
   count when snapshots start flowing.
4. Open DevTools console on the Ameyo tab — every captured network call is logged
   as `[WFM Ameyo] captured …`. Click **نسخ العيّنات** in the popup and share the
...
- `content_scripts` matches all URLs but **only activates** on a page that looks
  like Ameyo Live Monitoring (`looksLikeAmeyo()`), so it won't touch other sites.
  Once the Ameyo URL is known, narrow `matches`/`host_permissions` to that domain.
