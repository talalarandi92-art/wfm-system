...
import {
  ShieldCheck, Loader2, RefreshCw, CheckCircle2, AlertTriangle, XCircle, MinusCircle,
  Server, Calculator, ChevronDown,
} from 'lucide-react';
...
interface Check {
  id: string; category: 'technical' | 'calculation';
  label: string; labelAr: string; status: Status; count: number; detail: string; sample?: any[];
}
interface Report {
  generatedAt: string; score: number; status: Status;
...
const STAT = {
  ...
  warn: { color: '#f59e0b', Icon: AlertTriangle, ar: 'تحذير', en: 'Warn' },
...
export default function SystemHealthPage() {
  ...
  const load = useCallback(async () => {
    setL(true);
    try { const { data } = await apiClient.get<Report>('/health-guard'); setData(data); }
    catch { setData(null); }
    setL(false);
  }, []);
  useEffect(() => { load(); }, [load]);

  const groups: { key: 'technical' | 'calculation'; Icon: any; ar: string; en: string }[] = [
    { key: 'technical', Icon: Server, ar: 'صحّة النظام (فرونت/باك/بيانات)', en: 'System health (front/back/data)' },
    ...
    <div className="p-6 min-h-full" dir={ar ? 'rtl' : 'ltr'} style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.22)' }}>
            <ShieldCheck size={18} style={{ color: '#22c55e' }} />
          </div>
          <div>
            <h1 className="text-xl font-bold" style={{ color: tp(dark) }}>{ar ? 'حارس سلامة النظام' : 'System Integrity Guard'}</h1>
            <p className="text-xs" style={{ color: tsColor(dark) }}>{ar ? 'يتأكّد إنّ الفرونت/الباك شغّالين وإنّ الحسابات والقواعد تنطبق صح' : 'Verifies the platform is healthy and the schedule obeys the codified rules'}</p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20" style={{ color: '#475569' }}>
          <ShieldCheck size={32} className="mx-auto mb-3" style={{ color: '#334155' }} />
          <p className="text-sm">{ar ? 'تعذّر تشغيل الفحوصات' : 'Could not run checks'}</p>
        </div>
      ) : (
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-4 flex-wrap rounded-2xl px-5 py-4" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold" style={{ color: tp(dark) }}>{ar ? 'درجة السلامة' : 'Health score'}</p>
            ...
            <div className="flex items-center gap-2 flex-wrap ms-auto">
              {(['pass', 'warn', 'fail', 'skip'] as Status[]).map(s => {
                ...
                return (
                  <span key={s} className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg font-semibold" style={{ background: `${color}14`, color }}>
                    <Icon size={13} /> {data.counts[s]} {ar ? la : en}
                  </span>
                );
              })}
            </div>
            ...
            return (
              <div key={g.key}>
                <div className="flex items-center gap-2 mb-2">
                  <g.Icon size={15} style={{ color: '#818cf8' }} />
                  <h2 className="text-sm font-bold" style={{ color: tp(dark) }}>{ar ? g.ar : g.en}</h2>
                </div>
                  ...
                  {items.map(c => {
                    ...
                    return (
                      <div key={c.id} className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${color}22` }}>
                        <button onClick={() => hasSample && setOpen(o => ({ ...o, [c.id]: !o[c.id] }))}
                          className="w-full flex items-center gap-3 px-4 py-3 text-start"
                          style={{ cursor: hasSample ? 'pointer' : 'default' }}>
                          <Icon size={16} style={{ color, flexShrink: 0 }} />
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold" style={{ color: tp(dark) }}>{ar ? c.labelAr : c.label}</p>
                            <p className="text-[11px] truncate" style={{ color: '#64748b' }}>{c.detail}</p>
                          </div>
                          {c.count > 0 && (
                            <span className="text-[11px] font-bold px-2 py-0.5 rounded-lg" style={{ background: `${color}1a`, color }}>{c.count}</span>
                          )}
                                    ...
                                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                      {typeof row === 'object'
                                        ? Object.values(row).map((v, j) => (
                                          <td key={j} className="px-2 py-1 tabular-nums" style={{ color: j === 0 ? '#cbd5e1' : '#94a3b8', whiteSpace: 'nowrap' }}>{String(v)}</td>
                                        ))
                                        : <td className="px-2 py-1" style={{ color: '#f87171' }}>{String(row)}</td>}
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
