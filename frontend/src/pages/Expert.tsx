...
export default function ExpertPage() {
  ...
  /* theme-aware neutral tokens — dark keeps the original explicit values; light mirrors
     them with a slate-navy tint. Semantic/brand (emerald/amber/status) + mid-gray muted
  ...
  const T = {
    ...
    chipBg:  dark ? 'rgba(255,255,255,0.03)' : 'rgba(15,23,42,0.04)',
    ...
    <div className="p-6 min-h-full" dir={ar ? 'rtl' : 'ltr'} style={{ background: 'var(--bg)' }}>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: 'minmax(0,1.3fr) minmax(0,1fr)' }}>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: T.cardBg, border: `1px solid ${T.bdrSoft}` }}>
          <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: `1px solid ${T.bdrSoft}` }}><BookOpen size={14} style={{ color: '#10b981' }} /><span className="text-xs font-bold" style={{ color: tp(dark) }}>{ar ? 'قاعدة المعرفة' : 'Knowledge base'}</span></div>
          <div className="flex flex-wrap gap-1.5 px-4 py-3" style={{ borderBottom: `1px solid ${T.bdrRow}` }}>
            {topics.map(t => (
              <button key={t.topic} onClick={() => setActive(t.topic)} className="text-[11px] font-semibold px-2 py-1 rounded-lg" style={{ background: active === t.topic ? 'rgba(16,185,129,0.18)' : T.chipBg, color: active === t.topic ? '#34d399' : '#94a3b8', border: `1px solid ${active === t.topic ? 'rgba(16,185,129,0.3)' : 'transparent'}` }}>{ar ? t.titleAr : t.titleEn}</button>
            ))}
          </div>
          {activeEntry && (
            <div className="px-4 py-4">
              <h3 className="text-sm font-bold mb-2" style={{ color: tp(dark) }}>{ar ? activeEntry.titleAr : activeEntry.titleEn}</h3>
              <p className="text-[12px] leading-relaxed" style={{ color: T.text2 }}>{activeEntry.body}</p>
              <div className="flex flex-wrap gap-1 mt-3">{activeEntry.tags.map(tag => <span key={tag} className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: T.fieldBg, color: '#64748b' }}>{tag}</span>)}</div>
            </div>
          )}
        </div>

        {/* Ask */}
        <div className="rounded-2xl flex flex-col" style={{ background: T.cardBg, border: `1px solid ${T.bdrSoft}` }}>
          <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: `1px solid ${T.bdrSoft}` }}><Search size={14} style={{ color: '#10b981' }} /><span className="text-xs font-bold" style={{ color: tp(dark) }}>{ar ? 'اسأل الخبير' : 'Ask the expert'}</span></div>
          <div className="flex items-center gap-2 p-3">
            <input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && ask()} placeholder={ar ? 'مثلاً: كيف أحسب السعة للواتساب؟' : 'e.g. how to size WhatsApp capacity?'}
              className="flex-1 text-xs rounded-xl px-3 py-2 outline-none" style={{ background: T.fieldBg, border: `1px solid ${T.bdr}`, color: T.text }} />
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4">
            {asking ? <Loader2 size={16} className="animate-spin" style={{ color: '#10b981' }} /> : answer ? (
              <>
                ...
                <p className="text-[12px] whitespace-pre-line leading-relaxed" style={{ color: T.text2 }}>{answer}</p>
              </>
