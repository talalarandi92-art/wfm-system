...
export default function AgentScoresPage() {
  ...
  const agents = (d?.agents||[]).filter((a:any)=>{ const s=search.toLowerCase().trim(); return !s || a.name?.toLowerCase().includes(s) || String(a.person_no).includes(s); })
    .sort((a:any,b:any)=> sortBy==='net' ? ((b.netPoints??-1)-(a.netPoints??-1)) : (b.score-a.score));
    ...
    <div className="space-y-4 page-enter">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex rounded-lg overflow-hidden border border-white/10">
          {([['score',ar?'سكور الالتزام':'Adherence'],['net',ar?'Net Points':'Net Points']] as [any,string][]).map(([k,l])=>(
            <button key={k} onClick={()=>setSortBy(k)} className="px-2.5 py-1.5 text-[11px] font-semibold" style={sortBy===k?{background:'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff'}:{color:'#94a3b8'}}>{ar?'رتّب: ':''}{l}</button>
          ))}
        </div>
          ...
          <table className="w-full text-[11px]">
            ...
            <tbody>{agents.map((a:any)=>(
              <tr key={a.person_no} className="border-t border-white/5 hover:bg-white/[0.03] cursor-pointer" onClick={()=>nav('/agent-360')}>
                ...
                <td className="px-2 py-1.5 text-center" title={a.netPeriod||''}>{a.netPoints!=null?<span className="font-semibold" style={{ color:a.netPoints>=100?'#22c55e':a.netPoints>=80?'#06b6d4':a.netPoints>=60?'#f59e0b':'#f43f5e' }}>{a.netPoints}</span>:<span className="text-slate-600">—</span>}</td>
