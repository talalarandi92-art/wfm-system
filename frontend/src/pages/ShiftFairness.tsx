  ...
  return (
    <div className="space-y-4 page-enter">
      ...
      {!loading && s && (<>
        ...
        {tab === 'off' && (
          <div className="rounded-2xl p-4" style={panel}>
            <p className="text-[11px] mb-1" style={{ color: 'var(--text-2)' }}>{ar ? `من كل أوفات الموظف: كم نسبتها أيام عادية (رمادي) وكم ويك-إند خميس/جمعة/سبت (أخضر). "حصة الويك-إند" = كم % من إجمالي عطل نهاية الأسبوع بالفترة (${s.totalWeekendDays} يوم) أخذها أوف.` : `of each agent's OFF days: how many fall on weekdays (grey) vs weekend Thu/Fri/Sat (green). "weekend share" = % of all ${s.totalWeekendDays} weekend days in the period they got off.`}</p>
        ...
        {tab === 'weekend' && (
          <div className="rounded-2xl p-4" style={panel}>
            <div className="flex items-center gap-2 mb-1"><CalendarOff size={15} style={{ color: '#f59e0b' }} /><h3 className="text-sm font-bold" style={{ color: 'var(--text-1)' }}>{ar ? 'الأقل حظاً بالويك-إند أوف (الخميس/الجمعة/السبت)' : 'Fewest weekend (Thu/Fri/Sat) OFFs'}</h3></div>
