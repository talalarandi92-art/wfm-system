...
export class ScorecardGuardService implements OnModuleInit, OnModuleDestroy {
  ...
  async review(tid: string, week?: string, functionName?: string) {
    ...
    const pts = (r: any) => Number(r.net_points ?? 0);
    const overall = {
      count: rows.length,
      avg: +(rows.reduce((s, r) => s + pts(r), 0) / rows.length).toFixed(1),
      max: Math.max(...rows.map(pts)), min: Math.min(...rows.map(pts)),
    };
    ...
    for (const r of rows) { fnAvg.set(r.function_name, (fnAvg.get(r.function_name) ?? 0) + pts(r)); fnCount.set(r.function_name, (fnCount.get(r.function_name) ?? 0) + 1); }
    ...
    for (const r of rows) { const k = r.team_leader || '—'; const a = tlAgg.get(k) ?? { sum: 0, n: 0 }; a.sum += pts(r); a.n++; tlAgg.set(k, a); }
    const byTeamLeader = [...tlAgg.entries()].map(([tl, a]) => ({ teamLeader: tl, avg: +(a.sum / a.n).toFixed(1), count: a.n })).sort((a, b) => b.avg - a.avg);

    // Column maxes to normalize "weakest KPI".
    const maxes: Record<string, number> = {};
    for (const s of SCORE_FIELDS) maxes[s.f] = Math.max(1, ...rows.map(r => Number(r[s.f] ?? 0)));
    const weakest = (r: any) => {
      let best: { ar: string; ratio: number } | null = null;
      for (const s of SCORE_FIELDS) { const ratio = Number(r[s.f] ?? 0) / maxes[s.f]; if (best === null || ratio < best.ratio) best = { ar: s.ar, ratio }; }
      return best?.ar ?? '—';
    };

    const ranked = [...rows].sort((a, b) => pts(b) - pts(a));
    ...
    const belowTarget = rows.filter(r => pts(r) < (fnAvg.get(r.function_name)! / fnCount.get(r.function_name)!))
    ...
    return { week: wk, empty: false, overall, byFunction, byTeamLeader, top, bottom, belowTarget, coachingCandidates: belowTarget.length, trend };
  }
