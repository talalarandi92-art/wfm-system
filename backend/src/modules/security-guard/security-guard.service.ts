...
export class SecurityGuardService implements OnModuleInit, OnModuleDestroy {
  ...
  async run(tid: string): Promise<SecReport> {
    const checks: SecCheck[] = [];
    for (const fn of [
      () => this.failedLogins(tid),
      () => this.lockedAccounts(tid),
      () => this.stalePasswords(tid),
      () => this.dormantActive(tid),
      () => this.mustChange(tid),
      () => this.privilegedAdmins(tid),
      () => this.orphanUsers(tid),
      () => this.accessNotRevoked(tid),
      () => this.auditFreshness(tid),
      () => this.untraceableChanges(tid),
    ]) {
  ...
  // ── accounts ─────────────────────────────────────────────────────────────────
  private async failedLogins(tid: string): Promise<SecCheck> {
    ...
    return { id: 'failed_logins', category: 'accounts', label: 'Repeated failed logins', labelAr: 'محاولات دخول فاشلة متكرّرة', status: rows.length ? (hard ? 'fail' : 'warn') : 'pass', count: rows.length, detail: rows.length ? `${rows.length} حساب عليه محاولات فاشلة متكرّرة — احتمال محاولة اختراق` : 'لا محاولات دخول مشبوهة', sample: rows.map((r: any) => ({ email: r.email, attempts: r.failed_attempts })) };
  }

  private async lockedAccounts(tid: string): Promise<SecCheck> {
    const rows = await this.ds.query(
    ...
    return { id: 'locked_accounts', category: 'accounts', label: 'Currently locked accounts', labelAr: 'حسابات مقفولة حالياً', status: rows.length ? 'warn' : 'pass', count: rows.length, detail: rows.length ? `${rows.length} حساب مقفول — راجع سبب القفل` : 'لا حسابات مقفولة', sample: rows.map((r: any) => ({ email: r.email, until: r.locked_until })) };
  }

  private async stalePasswords(tid: string): Promise<SecCheck> {
    const [r] = await this.ds.query(
      `SELECT COUNT(*)::int n FROM users
        WHERE tenant_id = $1 AND status = 'active'
          AND (password_changed_at IS NULL OR password_changed_at < NOW() - INTERVAL '90 days')`, [tid]);
    const n = r?.n ?? 0;
    return { id: 'stale_passwords', category: 'accounts', label: 'Passwords older than 90 days', labelAr: 'كلمات مرور أقدم من 90 يوم', status: n ? 'warn' : 'pass', count: n, detail: n ? `${n} حساب نشط بكلمة مرور قديمة/غير محدّثة` : 'كل كلمات المرور محدّثة' };
  }

  private async dormantActive(tid: string): Promise<SecCheck> {
    const rows = await this.ds.query(
    ...
    return { id: 'dormant_active', category: 'accounts', label: 'Dormant but active accounts', labelAr: 'حسابات خاملة لكنها فعّالة', status: rows.length ? 'warn' : 'pass', count: rows.length, detail: rows.length ? `${rows.length} حساب نشط بلا دخول منذ 60+ يوم — يُفضّل تعطيله` : 'لا حسابات خاملة', sample: rows.map((r: any) => ({ email: r.email, lastLogin: r.last_login_at ?? 'أبداً' })) };
  }

  private async mustChange(tid: string): Promise<SecCheck> {
    const [r] = await this.ds.query(
      `SELECT COUNT(*)::int n FROM users WHERE tenant_id = $1 AND status = 'active' AND must_change_password = true`, [tid]);
    const n = r?.n ?? 0;
    return { id: 'must_change', category: 'accounts', label: 'Pending forced password change', labelAr: 'بانتظار تغيير كلمة مرور إجباري', status: n ? 'warn' : 'pass', count: n, detail: n ? `${n} حساب عليه تغيير إجباري لكلمة المرور لم يتم` : 'لا تغييرات إجبارية معلّقة' };
  }

  // ── access ───────────────────────────────────────────────────────────────────
  private async privilegedAdmins(tid: string): Promise<SecCheck> {
    const rows = await this.ds.query(
      `SELECT DISTINCT u.email FROM users u
         JOIN user_roles ur ON ur.user_id = u.id JOIN roles r ON r.id = ur.role_id
        WHERE u.tenant_id = $1 AND u.status = 'active' AND r.code = 'platform_admin' LIMIT 30`, [tid]);
    return { id: 'privileged_admins', category: 'access', label: 'Privileged admin accounts', labelAr: 'حسابات أدمن صلاحياتها واسعة', status: rows.length > 3 ? 'warn' : 'pass', count: rows.length, detail: rows.length > 3 ? `${rows.length} حساب أدمن — أكثر من اللازم، قلّل الصلاحيات الواسعة` : `${rows.length} حساب أدمن (ضمن المعقول)`, sample: rows.map((r: any) => ({ email: r.email })) };
  }

  private async orphanUsers(tid: string): Promise<SecCheck> {
    const rows = await this.ds.query(
      `SELECT email FROM users WHERE tenant_id = $1 AND status = 'active' AND employee_id IS NULL LIMIT 20`, [tid]);
    return { id: 'orphan_users', category: 'access', label: 'Accounts not linked to an employee', labelAr: 'حسابات غير مرتبطة بموظف', status: rows.length ? 'warn' : 'pass', count: rows.length, detail: rows.length ? `${rows.length} حساب نشط غير مربوط بموظف — راجع شرعيّته` : 'كل الحسابات مربوطة بموظفين', sample: rows.map((r: any) => ({ email: r.email })) };
  }

  private async accessNotRevoked(tid: string): Promise<SecCheck> {
    const rows = await this.ds.query(
    ...
    return { id: 'access_not_revoked', category: 'access', label: 'Active login for terminated/resigned staff', labelAr: 'وصول فعّال لموظف منتهٍ/مستقيل', status: rows.length ? 'fail' : 'pass', count: rows.length, detail: rows.length ? `${rows.length} موظف منتهٍ/مستقيل لا يزال حسابه فعّالاً — اسحب الوصول فوراً` : 'لا وصول غير مسحوب', sample: rows.map((r: any) => ({ emp: `${r.first_name_en} ${r.last_name_en}`, no: r.employee_no, status: r.status, email: r.email })) };
  }

  // ── audit ────────────────────────────────────────────────────────────────────
  private async auditFreshness(tid: string): Promise<SecCheck> {
    const [r] = await this.ds.query(
      `SELECT MAX(created_at) mx, EXTRACT(EPOCH FROM (NOW() - MAX(created_at)))/3600 hrs
         FROM audit_logs WHERE tenant_id = $1`, [tid]);
    if (!r?.mx) return { id: 'audit_freshness', category: 'audit', label: 'Audit trail activity', labelAr: 'نشاط سجلّ التدقيق', status: 'warn', count: 0, detail: 'لا يوجد سجلّ تدقيق' };
    const hrs = Math.round(Number(r.hrs));
    return { id: 'audit_freshness', category: 'audit', label: 'Audit trail activity', labelAr: 'نشاط سجلّ التدقيق', status: hrs <= 168 ? 'pass' : 'warn', count: 0, detail: `آخر إدخال تدقيق منذ ${hrs} ساعة` };
  }

  private async untraceableChanges(tid: string): Promise<SecCheck> {
    const [r] = await this.ds.query(
      `SELECT COUNT(*)::int n FROM audit_logs
        WHERE tenant_id = $1 AND actor_id IS NULL AND created_at > NOW() - INTERVAL '30 days'`, [tid]);
    const n = r?.n ?? 0;
    return { id: 'untraceable_changes', category: 'audit', label: 'Untraceable changes (no actor)', labelAr: 'تعديلات بلا منفّذ معروف', status: n ? 'warn' : 'pass', count: n, detail: n ? `${n} تعديل بآخر 30 يوم بلا منفّذ مسجّل` : 'كل التعديلات لها منفّذ مسجّل' };
  }
}
