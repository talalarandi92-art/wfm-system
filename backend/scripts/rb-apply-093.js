#!/usr/bin/env node
/* Apply migration 093 (Feb-2026 Offline AHT = the INBOUND band, D-081c) DIRECTLY — idempotent, no shared migrate runner.
...
(async () => {
  const file = '093_feb_offline_aht_band.sql';
  ...
  const rows = (await c.query(
    `SELECT r.kpi_code, c.function_name, c.weight, c.applies_from::text, c.applies_to::text, c.band->>'type' AS band_type
      ...
      WHERE c.function_name = 'Offline' AND r.kpi_code = 'AHT'
