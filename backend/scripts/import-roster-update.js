...
// Refresh from the new "update" drop. The comp-off & sick Odoo exports are NOT in
// that drop, so fall back to the complete ROSTER set for those two ONLY — otherwise
// every comp-off late/early exemption and Odoo-confirmed sick day would be lost and
// those employees wrongly flagged tardy/absent ("ما بنظلم حدا").
const DIR = process.env.ROSTER_DIR || 'C:/Users/t.bassam/Desktop/update';
const FALLBACK_DIR = process.env.ROSTER_FALLBACK_DIR || 'C:/Users/t.bassam/Desktop/ROSTER';
// Dry-run to a SCRATCH table by default; promote to the live roster_days only after the
// anchor comparison passes (set ROSTER_OUT_TABLE=roster_days to write live).
const OUT_TABLE = (process.env.ROSTER_OUT_TABLE || 'roster_days_scratch').replace(/[^a-z0-9_]/gi,'');
const SCHED_FILE = process.env.ROSTER_SCHED_FILE || 'CC Schedule 26 V3.0 (1).xlsx';
...
(async () => {
  ...
  console.log(`SOURCE=${DIR} | OUT=${OUT_TABLE} | window ${FROM}..${TO}`);
  ...
  const reader = new ExcelJS.stream.xlsx.WorkbookReader(path.join(DIR, SCHED_FILE), {});
  ...
  const punch = new Map();
  // Odoo Fingerprints.xlsx — cols Code|Date|Day|In|Out|Total|Late In|Early Out|OT|Status
  // = id(1) date(2) in(4) out(5) ot(9) status(10): identical layout to the old export.
  let wb = new ExcelJS.Workbook(); await wb.xlsx.readFile(path.join(DIR, 'Odoo Fingerprints.xlsx'));
  ...
  // Login Out Ameyo.xlsx — cols: 1=blank 2=Username(display) 3=User ID(login) 4=Login date
  // 5=Login time 6=Logout date 7=Logout time. CRITICAL: the new export stores the time-of-day as
  // TRUE UTC (login 16:04 reads as getUTCHours=12:57), so read minutes with LOCAL getHours (tmL) —
  // getUTCHours would shift every login 3h early and inflate OT. (Old export was local-as-UTC.)
  // Times are stored TRUE-UTC; convert to Kuwait local with a FIXED +3:00 (180 min). NOT getHours()
  // — that uses the 1899 epoch's historical LMT (+3:11) and overshoots by ~11 min/timestamp.
  const tmL = d => (d instanceof Date) ? (d.getUTCHours()*60 + d.getUTCMinutes() + 180) % 1440 : null;
  wb=new ExcelJS.Workbook(); await wb.xlsx.readFile(path.join(DIR,'Login Out Ameyo.xlsx')); ws=wb.worksheets[0];
  for(let r=2;r<=ws.rowCount;r++){ const row=ws.getRow(r); const login=str(row.getCell(3).value).toLowerCase(); if(!login)continue;
    const iso=isoOf(row.getCell(4)); if(!iso||iso<FROM||iso>TO)continue; addSys(sysByLogin,login,iso,tmL(row.getCell(5).value),tmL(row.getCell(7).value)); }
  // LoginandLogout Sprinklr.xlsx — sheet 'Login and Logout': 1=Agent (USERNAME e.g. 'e.mohamed' —
  // NOT the numeric Sprinklr id the old export used) 2=Login date 3=Login time 4=Logout date 5=Logout
  // time. The username matches the schedule's Username, so key into sysByLogin (like Ameyo); local times.
  wb=new ExcelJS.Workbook(); await wb.xlsx.readFile(path.join(DIR,'LoginandLogout Sprinklr.xlsx')); ws=wb.getWorksheet('Login and Logout')||wb.worksheets[0];
  for(let r=2;r<=ws.rowCount;r++){ const row=ws.getRow(r); const key=str(row.getCell(1).value).toLowerCase(); if(!key)continue;
    ...
    addSys(sysByLogin,key,iso,tmL(row.getCell(3).value),tmL(row.getCell(5).value)); }
  const annot = async (file, kind, dir=DIR) => { const m=new Map();
    if(!fs.existsSync(path.join(dir,file))){ console.log(`  ⚠ annot source missing, skipped: ${file}`); return m; }
    const w=new ExcelJS.Workbook(); await w.xlsx.readFile(path.join(dir,file));
    ...
    return m; };
  // permissions from the new drop; comp-off + sick fall back to the complete ROSTER set
  const perms=await annot('Attendance Permissions (attendance.permissions) (47).xlsx','perm'),
        comps=await annot('Compensatory Off (comp.off) ODOO.xlsx','comp',FALLBACK_DIR),
        sicks=await annot('Sick Leaves ODOO.xlsx','sick',FALLBACK_DIR);
